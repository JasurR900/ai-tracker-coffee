...
> **DEPRECATED.** Этот документ описывал отдельный auth (username/password) и
> собственный backend. Актуальный контракт — Point Coffee Core REST API:
> см. [`NUTRITION_MOBILE.md`](NUTRITION_MOBILE.md). Фронтенд использует JWT
> мобильного приложения Point Coffee; отдельный login/register больше нет.
